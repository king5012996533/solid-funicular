...
## 10. 输入框的参数集与「卡片↔卡片」引用链路（2026-09-26 二次取证）

> 用户明确要的是两件事：**卡片与卡片之间的交互**、**输入框的节奏和参数**。
> 这一节把这两件事的**原始选项集**抓下来（全部是点开弹窗后读到的原文）。

### 10.1 参数行 = 一行摘要 + 一个分组弹窗（"节奏"就在这里）

参数行上只有三段摘要：`[2.5] [全能参考] [16:9 · 720P · 30s · 1个]` + 右侧积分 + 发送。
点第三段，**一个弹窗里平铺五组参数**（原文）：
...
点第二段 `全能参考` —— 它其实是**视频生成模式选择器**（不是"参考方式"），八个模式：

```
文生视频 / 全能参考 / 图生视频 / 首尾帧 / 图片参考 / 视频编辑 / 视频续写 / 超长视频(Beta)
```

点第一段 —— **模型列表**，每行是「品牌 + 版本 + 时长/成本档 + 一句话卖点」：

```
Seedance 2.5            2min  最强视频模型，全能参考，30s音画同步
Seedance 2.5（样片模式） 1min  480P  低成本试片，满意后直出高清版本
Seedance 2.0            VIP/2min  最强视频模型，会员专属通道，15s音画同步
Wan 3.0                 3min  全模态参考，支持文档与网页输入，超写实高一致性生成
Wan 3.0 Prime           1min  超快速生成，全模态参考，超写实高一致性
Minimax H3              2min  全模态输入，多参数控制，多场景商用级生成
Minimax H3 Max          30s   后训练极速视频生成，支持文生、图生及首尾帧控制
```

> **「样片模式」是这轮最有商业价值的一条**：480P/1min 极低价试片，满意后直出高清版本。
> 它把"用户不敢花钱"这件事在设计层面解掉了 —— 和我们积分体系的痛点完全对上。

### 10.2 卡片↔卡片的引用链路：他们传什么、我们传什么

他们连线的**产物**是一个资产描述符（服务端 `nodeList[].data` 原文）：

```
imageList: [{ nodeId, url, label: "图片节点 - 副本", width, height, assetId, sourceCreatedAtMs }]
imageListOrder: ["i-IV8VjrEm1o"]
mixedListOrder: ["i-IV8VjrEm1o"]
```
...
| 维度 | LibTV | 我们 | 判断 |
|---|---|---|---|
| 上游产出作为下游输入 | 连线 → 资产描述符 | 连线 → `ReferenceableAsset`（`reference-resolver.ts`） | ✓ 骨架等价 |
| 溯源到来源卡片 | `nodeId` + **`label`（卡片名）** | `sourceNodeId` + `displayName` **都有** —— 但**芯片上只渲染 `@图片1`，不显示卡片名** | **差在渲染** |
| 引用顺序 | **显式持久化**（`imageListOrder`/`mixedListOrder`）+ 面板里 48×48 **可拖拽排序**缩略图 | 由**连线顺序实时推导**（`collectReferenceableAssets` 按 inboundEdges 顺序编号） | **缺持久化**：新增一条上游连线就可能让 `@图片1` 指向别的图，而历史提示词存的是纯文本 token |
| 上游产出时间 / 过期感知 | `sourceCreatedAtMs` + `isStale` + `_lastAppliedFullyAtMs` | 无 | **缺**：上游重跑后，下游不知道自己该重跑 |
| 资产稳定 id | `assetId` | 用 url 当标识 | 弱一档 |
| 引用语法 | `@图N` | `@图片1 / @视频1 / @文本1`（带种类名，更强） | ✓ 我们更好 |

**结论**：卡片间交互**骨架我们早就有了**（@引用、来源节点、按连线顺序编号），
缺的是三块**机制**——① 顺序持久化 + 拖拽排序；② 上游变化/过期的感知；③ 芯片上把来源卡片名露出来并可点击跳回。

### 10.3 关于「我们缺了什么参数」的实际结论

- **生成音频（开/关）**：我们**没有**（全仓 grep `enableSound` 零命中）。他们默认「开启」。
- **21:9**：我们**有**（`model-params.ts` 里三处档位）。
- **生成模式数量**：8 vs 我们的 3（文生视频/图生视频/首尾帧）—— 受上游能力限制，不是 UI 能补的。
- **「一行摘要 + 分组弹窗」的节奏**：**我们已经有了**
  （`VideoToolbar.vue` = `paramSummary` 摘要 + `SelectPopup` 里分组：生成方式 / 比例图形单选卡 / 视频时长滑杆+单位 s / 数量）。
- **样片模式**：没有，且需要服务端配合（低价档位 + 升级重跑）。
