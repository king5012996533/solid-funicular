...
export interface ContextMenuItem {
  ...
  icon?: string
  /**
   * 可选图标：直接给 SVG path 的 d（24×24 viewBox）。
   * 画布图标模块（components/icons/canvas-icons.ts）导出的就是这个格式，
   * 所以建节点这类菜单可以直接把节点图标带进来。
   */
  iconPath?: string
  /**
   * 上面那条 path 的描边色。必须是完整的 CSS 颜色值（可以是 var()）。
   * 走 style 绑定而不是 stroke 表现属性 —— 表现属性不解析 var()。
   */
  iconColor?: string
  /** 是否禁用 */
  disabled?: boolean
