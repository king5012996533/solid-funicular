...
<style scoped>
/*
 * 卡片外观收口：外壳由 CanvasConfigNodeShell 提供，但它还带着旧的
 * 16px 圆角 + 选中外发光 + 流光环。这里把 LLM 卡片拉回图片/视频/文本节点
 * 同一套：12px 圆角、常驻 1px 描边、选中只换描边颜色。
 * 选择器带上 .config-node-wrapper 提高一层权重，避免和外壳自身样式同权重竞争。
 */
.llm-config-node-outer :deep(.config-node-wrapper .config-node-card) {
  border-radius: 12px;
}
.llm-config-node-outer :deep(.config-node-wrapper .config-node-card.is-selected) {
  border-color: var(--canvas-node-border-selected);
  box-shadow: none;
}
.llm-config-node-outer :deep(.config-node-flow) {
  display: none;
}
.llm-config-node-outer :deep(.config-node-title) {
  font-size: 14px;
}
.llm-config-node-outer :deep(.config-node-title-input) {
  font-size: 14px;
}
.llm-config-node-outer :deep(.config-node-title:hover) {
