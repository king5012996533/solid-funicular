 ...
 * 预设工作流模板，支持一键添加到画布
 *
 * 形态说明：模板里只有「文本 / 图片 / 视频 / LLM」四类节点。
 * 以前文本和图片之间会插一个 imageConfig 节点专门放模型和尺寸，
 * 现在生成参数直接写在图片 / 视频节点自己的 data 上（model / size / quality / ratio / duration），
 * 也就是文本节点直接连到生成节点 —— 少一跳，和 libtv 的节点到节点一致。
 * 这里只固定「用哪个模型」，具体可选尺寸由该模型的能力决定，不写死。
 */
...
export const WORKFLOW_TEMPLATES: WorkflowTemplateDefinition[] = [
  // ========== 1. 多角度分镜 ==========
  {
    ...
    createNodes: (startPosition: WorkflowCanvasPosition) => {
      ...
      // 角色图：文本直连图片节点
      const imgId = getId()
      nodes.push({ id: imgId, type: 'image', position: { x: startPosition.x + sp, y: startPosition.y + rsp * 1.5 }, data: { url: '', label: '主角色图', ...imageParams() } })

      edges.push({ id: `e_${txtId}_${imgId}`, source: txtId, target: imgId, type: 'promptOrder', data: { promptOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })

      // 4个角度
      const ax = startPosition.x + sp * 2 + 100
      angles.forEach((key, i) => {
        ...
        const aImgId = getId()
        nodes.push({ id: aImgId, type: 'image', position: { x: ax + sp, y: ay }, data: { url: '', label: `${cfg.label} (${cfg.english})`, ...imageParams() } })

        edges.push({ id: `e_${tId}_${aImgId}`, source: tId, target: aImgId, type: 'promptOrder', data: { promptOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
        edges.push({ id: `e_${imgId}_${aImgId}`, source: imgId, target: aImgId, type: 'imageOrder', data: { imageOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
      })
  ...
  // ========== 2. 通用产品全套电商图 ==========
  {
    ...
    createNodes: (startPosition: WorkflowCanvasPosition) => {
      ...
      // 4组提示词 + 生成节点
      const prompts = [
        { label: '模特图提示词', content: '根据产品特性，生成一个适合展示该产品且时尚富有高级感的模特图，彩色人像，背景是白底，人物居中', nodeLabel: '生成模特图' },
        { label: '侧面展示图提示词', content: '根据产品图和产品信息，生成左侧侧面45度的展示图，高清展示侧面的产品形状和细节', nodeLabel: '侧面展示图' },
        { label: '俯瞰展示图提示词', content: '根据产品图和产品信息，生成从上往下俯瞰的产品展示图，高清展示俯瞰角度的产品形状和细节', nodeLabel: '俯瞰展示图' },
        { label: '拆解图提示词', content: '根据产品材质功能，生成一张产品核心部件的结构示意图，展现产品核心部件的内部构造', nodeLabel: '拆解图' }
      ]
      ...
      prompts.forEach((p, i) => {
        ...
        const gId = getId()
        nodes.push({ id: gId, type: 'image', position: { x: startPosition.x + col * 2, y: startPosition.y + i * row }, data: { url: '', label: p.nodeLabel, ...imageParams() } })
        ...
        // 产品图 → 生成节点
        edges.push({ id: `e_${imgId}_${gId}`, source: imgId, target: gId, type: 'imageOrder', data: { imageOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
      })
  ...
  // ========== 3. 文生图基础工作流 ==========
  {
    ...
    name: '文生图基础',
    description: '最简单的文本到图片生成工作流：文本提示词直连图片节点',
    category: 'basic',
    createNodes: (startPosition: WorkflowCanvasPosition) => {
      _counter = 0
      const nodes: WorkflowTemplateNode[] = [], edges: WorkflowCanvasEdge[] = []
      ...
      const imgId = getId()
      nodes.push({ id: imgId, type: 'image', position: { x: startPosition.x + 400, y: startPosition.y }, data: { url: '', label: '文生图', ...imageParams() } })

      edges.push({ id: `e_${tId}_${imgId}`, source: tId, target: imgId, type: 'promptOrder', data: { promptOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
  ...
  // ========== 4. 图生视频工作流 ==========
  {
    ...
    createNodes: (startPosition: WorkflowCanvasPosition) => {
      ...
      const vId = getId()
      nodes.push({ id: vId, type: 'video', position: { x: startPosition.x + 800, y: startPosition.y }, data: { url: '', duration: 0, label: '图生视频', feature: 'image-to-video' } })

      edges.push({ id: `e_${tId}_${imgId}`, source: tId, target: imgId, type: 'promptOrder', data: { promptOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
      edges.push({ id: `e_${imgId}_${vId}`, source: imgId, target: vId, type: 'imageRole', data: { imageRole: 'first_frame_image' }, sourceHandle: 'right', targetHandle: 'left' })
  ...
  // ========== 5. 短剧角色设计 ==========
  {
    ...
    createNodes: (startPosition: WorkflowCanvasPosition) => {
      ...
      edges.push({ id: `e_${styleRefId}_${frontImgId}`, source: styleRefId, target: frontImgId, type: 'imageOrder', data: { imageOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
      ...
      variants.forEach((v, i) => {
        ...
        const gId = getId()
        nodes.push({ id: gId, type: 'image', position: { x: vx + col, y: vy }, data: { url: '', label: v.nodeLabel, ...imageParams(v.size) } })
        edges.push({ id: `e_${frontImgId}_${gId}`, source: frontImgId, target: gId, type: 'imageOrder', data: { imageOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
        edges.push({ id: `e_${tId}_${gId}`, source: tId, target: gId, type: 'promptOrder', data: { promptOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
      })
  ...
  // ========== 6. 多时段场景背景 ==========
  {
    ...
    createNodes: (startPosition: WorkflowCanvasPosition) => {
      ...
      edges.push({ id: `e_${basePromptId}_${baseImgId}`, source: basePromptId, target: baseImgId, type: 'promptOrder', data: { promptOrder: 2 }, sourceHandle: 'right', targetHandle: 'left' })

      // 第二阶段：多时段变体
      const variants = [
        { label: '傍晚场景提示词', content: '参考提供的基础场景图，保持场景构图、建筑、环境元素完全一致，仅改变光照为傍晚时段：夕阳西下，天空呈橙红色渐变，光线柔和温暖，建筑投射长影', nodeLabel: '傍晚场景' },
        { label: '夜晚场景提示词', content: '参考提供的基础场景图，保持场景构图、建筑、环境元素完全一致，仅改变光照为夜晚时段：霓虹灯亮起，城市灯光璀璨，天空深蓝或黑色，窗户透出暖光', nodeLabel: '夜晚场景' },
        { label: '雨天场景提示词', content: '参考提供的基础场景图，保持场景构图、建筑、环境元素完全一致，仅改变天气为雨天：细雨绵绵，地面湿润有倒影，天空阴沉灰暗，氛围忧郁', nodeLabel: '雨天场景' }
      ]
  ...
  // ========== 7. 绘本生成器 ==========
  {
    ...
    createNodes: (startPosition: WorkflowCanvasPosition) => {
      ...
      nodes.push({ id: char1ImgId, type: 'image', position: { x: startPosition.x + col * 3, y: startPosition.y - row * 1.5 }, data: { url: '', label: '主角色1设计图', ...imageParams() } })
      ...
      pages.forEach((p, i) => {
        ...
        const pImgId = getId()
        nodes.push({ id: pImgId, type: 'image', position: { x: startPosition.x + col * 4, y: py }, data: { url: '', label: `第${i + 1}页插图`, ...imageParams() } })
        edges.push({ id: `e_${pTxtId}_${pImgId}`, source: pTxtId, target: pImgId, type: 'promptOrder', data: { promptOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
        edges.push({ id: `e_${char1ImgId}_${pImgId}`, source: char1ImgId, target: pImgId, type: 'imageOrder', data: { imageOrder: 1 }, sourceHandle: 'right', targetHandle: 'left' })
        if (i >= 2) {
          edges.push({ id: `e_${char2ImgId}_${pImgId}`, source: char2ImgId, target: pImgId, type: 'imageOrder', data: { imageOrder: 2 }, sourceHandle: 'right', targetHandle: 'left' })
        }
      })
