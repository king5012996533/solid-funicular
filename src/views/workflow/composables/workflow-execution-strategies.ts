...
const executeTextToImage = async (
  ...
  // 文本直接连到图片节点。图片节点自己带模型与尺寸参数，
  // 拿到上游文本后自动执行，中间不再有「配置节点」这一跳。
  const textId = addNode('text', { x: position.x, y: position.y }, { content: imagePrompt, label: '图片提示词' })
  const imageId = addNode('image', { x: position.x + spacing, y: position.y }, { label: '文生图', autoExecute: true })
  addEdge({ source: textId, target: imageId, sourceHandle: 'right', targetHandle: 'left', type: 'promptOrder', data: { promptOrder: 1 } })
...
const executeTextToImageToVideo = async (
  ...
  const imageNodeId = addNode('image', { x: position.x + spacing, y: position.y }, { label: '文生图', autoExecute: true })
  addEdge({ source: imageTextId, target: imageNodeId, sourceHandle: 'right', targetHandle: 'left', type: 'promptOrder', data: { promptOrder: 1 } })

  try {
    context.addLog('info', '等待图片生成完成...')
    await context.waitForNodeReady(imageNodeId)
    ...
    const videoNodeId = addNode('video', { x: videoX, y: position.y + 200 }, { label: '图生视频', autoExecute: true })
    addEdge({ source: videoTextId, target: videoNodeId, sourceHandle: 'right', targetHandle: 'left', type: 'promptOrder', data: { promptOrder: 1 } })
    addEdge({ source: imageNodeId, target: videoNodeId, sourceHandle: 'right', targetHandle: 'left', type: 'imageRole', data: { imageRole: 'first_frame_image' } })

    context.addLog('success', '文生图生视频工作流已启动')
    return { imageTextId, videoTextId, imageNodeId, videoNodeId }
  } catch (error: unknown) {
...
const executeStoryboard = async (
  ...
  try {
    ...
    const characterImageId = addNode('image', { x: position.x + spacing, y: position.y }, { label: '角色参考图', autoExecute: true })
    addEdge({ source: characterTextId, target: characterImageId, sourceHandle: 'right', targetHandle: 'left', type: 'promptOrder', data: { promptOrder: 1 } })

    context.addLog('info', '等待角色参考图生成...')
    await context.waitForNodeReady(characterImageId)
    ...
    for (let index = 0; index < shotCount; index += 1) {
      ...
      const shotImageId = addNode('image', { x: position.x + spacing, y: shotY }, { label: `分镜${index + 1}`, autoExecute: true })
      addEdge({ source: shotTextId, target: shotImageId, sourceHandle: 'right', targetHandle: 'left', type: 'promptOrder', data: { promptOrder: 1 } })
      addEdge({ source: characterImageId, target: shotImageId, sourceHandle: 'right', targetHandle: 'left', type: 'imageOrder', data: { imageOrder: 1 } })

      context.addLog('info', `等待分镜${index + 1}生成...`)
      await context.waitForNodeReady(shotImageId)
      context.addLog('success', `分镜${index + 1}已生成`)
      createdShots.push({ textId: shotTextId, imageId: shotImageId })
    }

    context.addLog('success', `分镜工作流完成，共生成 ${shotCount} 个分镜`)
    return { characterTextId, characterImageId, shots: createdShots }
  } catch (error: unknown) {
...
const executeMultiAngleStoryboard = async (
  ...
  try {
    ...
    const createdAngles: Array<{ key: string; textId: string; imageId: string }> = []
    ...
    for (let index = 0; index < angles.length; index += 1) {
      ...
      const imageId = addNode('image', { x: angleX + spacing, y: angleY }, { label: `${config.label} (${config.english})` })
      addEdge({ source: textId, target: imageId, sourceHandle: 'right', targetHandle: 'left', type: 'promptOrder', data: { promptOrder: 1 } })
      addEdge({ source: characterImageId, target: imageId, sourceHandle: 'right', targetHandle: 'left', type: 'imageOrder', data: { imageOrder: 1 } })

      createdAngles.push({ key, textId, imageId })
    }

    context.addLog('success', '多角度分镜工作流已创建，请上传主角色图后点击各图片节点的"立即生成"按钮')
