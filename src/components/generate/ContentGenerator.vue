...
interface Props {
  ...
  /**
   * 参数初值（工作流节点把自己的 model / size / quality 传进来）。
   * 用于让节点卡片上的参数 chip 与工具栏保持一致：节点是参数的持久化处，
...
export interface GeneratorParamsSnapshot {
  modelKey?: string
  ratio?: string
  resolution?: string
  count?: number
...
const emit = defineEmits<{
  ...
  /** 工具栏参数变化（工作流节点据此把参数写回节点 data，卡片上就能显示 chip） */
 ...
 * 工具栏参数的两向同步。
 *
 * 分工：工作流节点把参数持久化在节点 data 上（卡片 chip 显示的就是它），
 * 工具栏是参数的编辑入口。任何一边改动都以「值不同才写」的方式同步，
 * 所以不会出现互相触发导致的重渲染循环。
 */
const readToolbarParams = (): GeneratorParamsSnapshot | null => {
  if (currentType.value === 'image') {
    const toolbar = imageToolbarRef.value as ExposedImageToolbarInstance | null
    ...
    return {
      modelKey: toolbar.currentModelVersion,
      ratio: toolbar.currentSize,
      resolution: toolbar.currentQuality,
      count: Number(toolbar.currentCount) || 1,
    }
  }
  ...
  if (currentType.value === 'video') {
    ...
    return {
      modelKey: toolbar.currentModelVersion,
      ratio: toolbar.currentSize,
      resolution: toolbar.currentResolution,
      duration: toolbar.currentDuration,
      feature: toolbar.currentFeature,
    }
  }
...
const currentParams = computed<GeneratorParamsSnapshot | null>(() => readToolbarParams())

const applyInitialParams = (params?: GeneratorParamsSnapshot) => {
  if (!params) return

  if (currentType.value === 'image') {
    const toolbar = imageToolbarRef.value as ExposedImageToolbarInstance | null
    if (!toolbar) return
    // 逐项比较后再写：初值与工具栏一致时保持不动，避免把工具栏刚算好的默认值顶掉
    if (params.modelKey && params.modelKey !== toolbar.currentModelVersion) {
      toolbar.currentModelVersion = params.modelKey
    }
    if (params.ratio && params.ratio !== toolbar.currentSize) {
      toolbar.currentSize = params.ratio
    }
    if (params.resolution && params.resolution !== toolbar.currentQuality) {
      toolbar.currentQuality = params.resolution
    }
    return
  }
  ...
  if (currentType.value === 'video') {
    ...
    if (params.resolution && params.resolution !== toolbar.currentResolution) {
      toolbar.currentResolution = params.resolution
    }
...
defineExpose({
  ...
  applyDraft,
  submitDraft,
  getCurrentParams: () => currentParams.value,
})
...
// 「高级设置」只在这些模式下有意义：参考图（图片 / Agent）与视频首尾帧是仅有的、
// 真正会随 send 提交的次要参数；其它模式没有可放的参数，直接不渲染按钮。
const showAdvancedParams = computed(() => {
  if (currentType.value === 'image') return true
  if (currentType.value === 'agent') return !props.hideImageUpload
  return currentType.value === 'video'
})
        ...
        <div class="container-yMr4oW container-vUEhlb toolbar-settings-YNMCja toolbar-settings-hsPiGw">
          <div class="content-BF8rbZ content-Rvn0mS toolbar-settings-content-uImXGN toolbar-settings-content-AqQb52">
            ...
            <template v-else>
              ...
              <!-- 高级设置：次要参数（参考图 / 视频首尾帧），紧挨参数摘要 -->
              <AdvancedParamsPopover
                v-if="showAdvancedParams"
                :mode="currentType"
                :reference-images="imageReferenceImages"
                :first-frame="videoFirstFrameImage"
                :last-frame="videoLastFrameImage"
                :reference-limit="IMAGE_REFERENCE_LIMIT"
                :placement="popupPlacement"
                @add-references="handleImageReferenceChange"
                @remove-reference="removeImageReference"
                @first-frame-change="handleVideoFirstFrameChange"
...
/* ========== 参数页脚（图片 / 视频工具栏共用） ==========
   一行摘要 + 点开的参数面板；摘要与面板都走 §1 token，不写死色值。 */

.toolbar-summary-trigger {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  height: 32px;
...
.toolbar-summary-trigger.compact {
  height: 28px;
  padding: 0 8px;
...
.toolbar-summary-trigger:hover,
.toolbar-summary-trigger:focus-visible {
  background: var(--bg-block-secondary-hover);
  color: var(--text-primary);
}

.toolbar-summary-trigger:active {
  background: var(--bg-block-secondary-pressed);
}

.toolbar-summary-chevron {
  flex-shrink: 0;
  width: 14px;
  height: 14px;
  color: var(--text-tertiary);
...
/* 参数面板用悬浮件底 + 8px 圆角；`:root .lv-select-popup` 是全局菜单规则，
   这里必须带上同级的类选择器才压得住它的 16px 圆角与下拉底色。 */
:root .lv-select-popup.generator-param-popup {
  background: var(--canvas-float-block-default);
  border: 1px solid var(--stroke-secondary);
  border-radius: 8px;
}

.generator-param-panel {
  display: flex;
...
.generator-param-row-label {
  color: var(--text-tertiary);
  font-size: 12px;
  line-height: 1;
}

.generator-param-chips {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
}

.generator-param-chip {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  height: 28px;
...
.generator-param-chip:hover,
.generator-param-chip:focus-visible {
  background: var(--bg-block-secondary-hover);
  color: var(--text-primary);
}

.generator-param-chip.is-active {
  border-color: var(--brand-main-default);
  background: var(--bg-block-secondary-default);
  color: var(--brand-main-default);
}

.generator-param-chip-hint {
  color: var(--text-tertiary);
  font-size: 12px;
}
